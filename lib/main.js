...
const bump_package_version_1 = __importDefault(require("./bump-package-version"));
const create_github_release_1 = __importDefault(require("./create-github-release"));
const BUMPS = [false, "patch", "minor", "major", "init"];
function run() {
    var e_1, _a, e_2, _b;
        ...
        try {
            ...
            try {
                for (var _e = __asyncValues(Object.keys(byPkg)), _f; _f = yield _e.next(), !_f.done;) {
                    let pkgDir = _f.value;
                    const tag = yield bump_package_version_1.default(pkgDir, byPkg[pkgDir].increment);
                    yield create_github_release_1.default(tag, byPkg[pkgDir].commits);
                }
            }
